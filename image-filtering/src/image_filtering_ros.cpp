...
ImageFilteringNode::ImageFilteringNode(const rclcpp::NodeOptions & options) : Node("image_filtering_node", options) 
{
...
void ImageFilteringNode::set_filter_params(){
    ...
    params.flip.flip_code = this->get_parameter("filter_params.flip.flip_code").as_int();
...
//     image_pub_->publish(*message);
    ...
    cv_bridge::CvImage(msg->header, "bgr8", filtered_image).toImageMsg(*message);
