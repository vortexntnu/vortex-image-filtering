 ...
 * Flips the image
 */
void flip_filter(const FilterParams& params, const cv::Mat &original, cv::Mat &modified);

/**
 * Makes edges harder
 */
void sharpening_filter(const FilterParams& params, const cv::Mat &original, cv::Mat &modified);

/**
 * Makes edges harder in a smarter way
 */
void unsharpening_filter(const FilterParams& params, const cv::Mat &original, cv::Mat &modified);

/**
 * Expands the dark areas of the image
 */
void eroding_filter(const FilterParams& params, const cv::Mat &original, cv::Mat &modified);

/**
 * Expands the bright areas of the image
 */
void dilating_filter(const FilterParams& params, const cv::Mat &original, cv::Mat &modified);
